...
//  Blocstagram
//
//  Created by Melissa Boring on 11/6/15.
...
- (void) layoutSubviews {
    ...
    //The text view won't draw text that intersects with this path, causing it to wrap
    self.textView.textContainer.exclusionPaths = @[buttonPath];
}

//if no longer writing dismiss keyboard
...
//isWriting change will call update the views
...
{
    if (self.isWritingComment)
        //done writing send comment to API
    {
        [self.textView resignFirstResponder];
